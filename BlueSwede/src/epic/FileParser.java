...
public class FileParser {
	...
	public static void saveMapFile() {
		...
		fileContent.append("MapsizeX:" + Main.getMapTilesX() + "MapsizeY:" + Main.getMapTilesY());
		fileContent.append("PlacedObjects");
		for (Placeable object : Main.getAllPlacedObjects()) {
			fileContent.append(object);
			fileContent.append("#");
		}

		fileContent.append("TerrainObjects");
		for (int i = 0; i < Main.getMapTilesX(); i++) {
			for (int j = 0; j < Main.getMapTilesY(); j++) {
				fileContent.append("X" + i + "Y" + j + Main.getTerrain()[i][j]);
				fileContent.append("#");
			}
		}
